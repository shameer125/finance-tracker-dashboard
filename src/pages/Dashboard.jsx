...
export default function Dashboard() {
  ...
  const dynamicSavings =
    dynamicIncome > dynamicExpenses ? dynamicIncome - dynamicExpenses : 0;
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div
          className="glass-panel p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 
        ...
        >
          ...
          {payload.map((pld, index) => (
            ...
            >
              {pld.name}: $
              {pld.value.toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </p>
          ))}
        </div>
      );
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-center justify-between bg-white/40 dark:bg-slate-950/20 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div
          variants={itemVariants}
          className="glass-card p-6 rounded-3xl lg:col-span-2 flex flex-col 
        justify-between"
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Aggregated inflow vs expense curves ({timeRange})
              </p>
            </div>
        ...
        >
          <div>
            ...
            <p className="text-xs text-slate-400">
              Category split during period ({timeRange})
            </p>
          </div>
            ...
            {pieData.length > 0 && (
              <div className="absolute text-center">
                ...
                <span className="block text-xl font-extrabold text-slate-800 dark:text-white">
                  $
                  {dynamicExpenses.toLocaleString("en-US", {
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {pieData.slice(0, 4).map((entry) => (
              <div key={entry.name} className="flex items-center gap-1.5">
                ...
                <span className="font-bold text-slate-800 dark:text-slate-200 ml-auto">
                  ${entry.value}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div
          variants={itemVariants}
          className="glass-card p-6 rounded-3xl lg:col-span-2"
        >
          ...
          <div className="space-y-4 py-3 flex-1 overflow-y-auto max-h-60 mt-2 pr-1">
            {savingsGoals.map((goal) => {
              ...
              return (
                <div
                  key={goal.id}
                  className="space-y-1.5 p-2.5 rounded-2xl hover:bg-slate-100/40 dark:hover:bg-slate-900/20 transition-all border border-transparent"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          <div>
            <h3 className="font-bold text-slate-800 dark:text-white text-base">
              Quick Transaction Record
            </h3>
            <p className="text-xs text-slate-400">
              Instantly register a payment details
            </p>
          </div>
        ...
        <motion.div
          variants={itemVariants}
          className="glass-card p-6 rounded-3xl flex flex-col justify-between bg-indigo-500/5 border border-indigo-500/10 dark:border-indigo-500/20"
        >
          <div>
            <h3 className="font-bold text-slate-800 dark:text-indigo-200 text-base">
              FinSaaS AI Engine
            </h3>
            <p className="text-xs text-slate-400">
              Automated ledger warnings & tips
            </p>
          </div>

          <div className="space-y-3.5 my-4">
            <div className="flex gap-2 text-xs">
              <span className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-500 h-8 self-start">
                ⚡
              </span>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
                <strong>Savings goal progress:</strong> Your Emergency Reserve
                goal is{" "}
                {(
                  ((savingsGoals.find((g) => g.id === "goal-2")?.current || 0) /
                    (savingsGoals.find((g) => g.id === "goal-2")?.target ||
                      10000)) *
                  100
                ).toFixed(0)}
                % completed. Fund $150 more to secure safety reserves!
              </p>
            </div>
            <div className="flex gap-2 text-xs">
              <span className="p-1.5 rounded-lg bg-rose-500/10 text-rose-500 h-8 self-start">
                ⚠️
              </span>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
                <strong>Budget limit caution:</strong> Food spent is reaching
                84% of your configured monthly threshold. We suggest eating at
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800/80 mb-4">
                <div>
                  <h3 className="font-bold text-slate-800 dark:text-white text-sm">
                    Fund "{selectedGoal.name}"
                  </h3>
                  <p className="text-xs text-slate-400">
                    Allocate checking balance to goal targets
                  </p>
                </div>
                <button
                  onClick={() => setSelectedGoal(null)}
            ...
            >
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800/80 mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Establish a new target reserve
                  </p>
                </div>
                <button
                  onClick={() => setNewGoalModal(false)}
